...
- `apps/frontend`：前端应用，使用 Vite、React、TypeScript 和 Ant Design，实现用户名密码登录、带二次密码确认的注册、启动恢复当前用户和退出登录。
- `apps/backend`：后端服务，使用 Node.js、Express、TypeScript 和 MySQL，提供健康检查、JWT 用户认证、AI 聊天会话和消息接口，并通过 Vitest + Supertest 覆盖 HTTP + MySQL e2e 测试。
- `docs`：接口说明和项目文档，包含已实现的用户认证接口和 AI 聊天接口设计。

当前前后端已基于 `docs/api/auth.md` 实现用户名密码注册、登录、恢复身份、退出登录的 JWT 认证闭环。
AI 聊天功能已在 `docs/api/chat.md` 中完成第一版接口设计，后端已实现会话管理、历史消息查询和基于 OpenAI SDK 的 SSE 消息发送接口，模型通过 `OPENAI_MODEL` 配置；前端功能尚未实现。
